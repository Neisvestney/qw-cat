  ...
  useEffect(() => {
    const handleTimeUpdate = (e: Event) => {
      if (appStateStore.currentVideo)
        appStateStore.currentVideo.handleVideoStateChange(
          "time",
    ...
    const handleVideoPause = (e: Event) => {
      if (appStateStore.currentVideo)
        appStateStore.currentVideo.handleVideoStateChange("playing", false);
    };

    const handleWaiting = (e: Event) => {
      if (appStateStore.currentVideo)
        appStateStore.currentVideo.handleVideoStateChange("loading", true);
    };
    ...
    const dispose = autorun(() => {
      if (videoElement == null || appStateStore.currentVideo == null) return;

      if (appStateStore.currentVideo.videoTargetState.time != null)
        videoElement.currentTime = appStateStore.currentVideo.videoTargetState.time;

      if (appStateStore.currentVideo.videoTargetState.playing != null) {
        if (appStateStore.currentVideo.videoTargetState.playing) {
          videoElement.play();
        } else {
          videoElement.pause();
        }
              ...
              <Stack direction={"row"} spacing={1} alignItems={"center"}>
                ...
                <Stack>
                  ...
                  <Typography variant={"subtitle1"}>
                    Probably video codec unsupported - You can still trim the video if ffmpeg
      ...
      <Controls>
        ...
        <FormGroup sx={{minHeight: 84}}>
          {appStateStore.currentVideo.audioStreams.map((audioStream, index) => {
            ...
            const defaultAudio =
              audioStream.streamIndex == appStateStore.currentVideo!.defaultAudioStreamIndex;
            ...
            return (
              ...
              >
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={() =>
                        appStateStore.currentVideo!.toggleAudioStream(audioStream.streamIndex)
                      }
                    />
                  }
                  label={
                ...
                />
                {(defaultAudio || audioStreamPath) && (
                  <AudioVolumeSlider audioStream={audioStream} />
                )}
              </Stack>
            );
          })}
...
const TimelineWithControls = observer(() => {
  ...
  return (
    <Box sx={{display: "flex", alignItems: "end", gap: 1, width: "100%"}}>
      <Box sx={{marginBottom: "2px"}}>
        <IconButton onClick={appStateStore.currentVideo?.toggleVideoPlaying}>
          {appStateStore.currentVideo.videoState.playing ? <PauseIcon /> : <PlayArrowIcon />}
        </IconButton>
      </Box>
      <Timeline />
      <Box sx={{marginBottom: "8px", marginLeft: 1.3}}>
        {formatTime(
          appStateStore.currentVideo.videoState.time ?? 0,
          appStateStore.currentVideo.duration ?? 0,
        )}
      </Box>
      <Box sx={{marginBottom: "2px"}}>
...
const Timeline = observer(() => {
  ...
  const sliderValue = [
    appStateStore.currentVideo.trimStart ?? 0,
    appStateStore.currentVideo.trimEnd ?? 0,
  ];
  ...
  return (
    <Box sx={{display: "grid", marginTop: 4, flex: "1", marginLeft: "12px", marginRight: "12px"}}>
...
const TimelineTrimThumbComponent = observer(function TimelineTrimThumbComponent(
...
const VideoProgressThumbComponent = observer(function VideoProgressThumbComponent(
  props: React.HTMLAttributes<unknown>,
) {
  const {children, ...other} = props;
  return (
...
const AudioVolumeSlider = observer(({audioStream}: {audioStream: AudioStream}) => {
  ...
  const onInputValueChanged = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    appStateStore.currentVideo?.updateAudioStreamGain(
  ...
  return (
    <Stack spacing={2} direction="row" sx={{alignItems: "center", width: 400}}>
      <VolumeDown />
      <Slider
        value={audioStream.gain}
        onChange={(e, v) =>
          appStateStore.currentVideo?.updateAudioStreamGain(audioStream.streamIndex, v)
        }
        aria-label="Volume"
    ...
    <Stack direction={"row"} spacing={1}>
      <Tooltip title="Hotkey: [A]" disableInteractive>
        <Button
          ...
          onClick={() => appStateStore.currentVideo?.handleStartHere()}
        >
          Start Here
        </Button>
      </Tooltip>
      <Tooltip title="Hotkey: [D]" disableInteractive>
        <Button
          ...
          onClick={() => appStateStore.currentVideo?.handleEndHere()}
        >
          End Here
        </Button>
      </Tooltip>
      <Tooltip title="Hotkey: [R]" disableInteractive>
        <Button
          variant={"outlined"}
          onClick={() => appStateStore.currentVideo?.handlePlayFromStart()}
...
const VideoPlayPauseBlinker = observer(() => {
  ...
  return (
    <VideoPlayPauseBlinkerWrapper className={blinkPlayPauseIcon ? "blink" : ""}>
      {appStateStore.currentVideo.videoState.playing ? (
